...
exports.removeFromWishlist = exports.addToWishlist = exports.getUserWishlist = exports.getTopSellingProducts = exports.getAllProducts = exports.getProduct = void 0;
...
const getAllProducts = async (req, res) => {
    try {
        const { name, category, seasonCollection, brand, minPrice, maxPrice, sortBy, order, page = 1, limit = 10, isAvailable, } = req.query;
        ...
        if (seasonCollection)
            query.seasonCollection = seasonCollection;
        if (brand)
            query.brand = brand;
...
const getUserWishlist = async (req, res) => {
    ...
    try {
        ...
        const total = await User_1.default.findById(userId).select("wishList");
...
const addToWishlist = async (req, res) => {
    try {
        ...
        const updatedUser = await User_1.default.findByIdAndUpdate(req.user?.userId, { $addToSet: { wishList: productId } }, { new: true })
            .select("wishList")
            .populate({
            path: "wishList",
...
const removeFromWishlist = async (req, res) => {
    try {
        ...
        const updatedUser = await User_1.default.findByIdAndUpdate(req.user?.userId, { $pull: { wishList: productId } }, { new: true })
            .select("wishList")
            .populate({
            path: "wishList",
            select: "name price images",
