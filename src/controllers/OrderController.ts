...
const useCoupon = async (
  ...
  if (discount > totalPrice) throw new Error("Discount exceeds total price");
...
export const makeOrder = async (req: Request, res: Response): Promise<void> => {
  try {
    const { products, coupon, deliveryMethod, deliveryInfo }: IOrderInfo =
      req.body;

    const pricePromises = products.map(
...
export const cancelMyOrder = async (
  ...
  try {
    ...
    if (reason) order.deliveryInfo.notes = reason;
